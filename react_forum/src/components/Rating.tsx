      ...
      if (previouslyRated) {
        if (!likeState && !dislikeState) {
          const response = await axiosPrivate.delete(
            RATING_URL + 'delete.php?type=' + props.type,
            {
        ...
        if (likeState || dislikeState) {
          const response = await axiosPrivate.put(
            RATING_URL + 'update.php?type=' + props.type,
            JSON.stringify({
              id: props.id,
              thumbsUp: likeState,
        ...
        const response = await axiosPrivate.post(
          RATING_URL + 'create.php?type=' + props.type,
          JSON.stringify({
            id: props.id,
            thumbsUp: likeState,
            thumbsDown: dislikeState,
  ...
  useEffect(() => {
    if (liked) {
      setLikeCounter((value) => {
        return previouslyLiked ? props.thumbsUps : Number(value) + 1;
      });
      setDisliked(false);
    } else
      setLikeCounter((value) => {
        return previouslyLiked ? Number(value) - 1 : props.thumbsUps;
      });
  }, [liked]);
  ...
  useEffect(() => {
    if (disliked) {
      setDislikeCounter((value) => {
        return previouslyDisliked ? props.thumbsDowns : Number(value) + 1;
      });
      setLiked(false);
    } else
      setDislikeCounter((value) => {
        return previouslyDisliked ? Number(value) - 1 : props.thumbsDowns;
      });
  }, [disliked]);
